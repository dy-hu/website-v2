                ...
                {({size}) => {
                    ...
                    className='w-max flex items-center justify-center relative hover:pause animate-spin-slow group'>
                    {
                        BtnList.map((btn, index) => {
                            ...
                            return <NavButton className='absolute' key={btn.label} x={x} y={y} {...btn}/>
                        })}
                    </motion.div>
                        ...
                        <>
                            ...
                            className='w-full px-2.5 xs:p-0 xs:w-max flex flex-col space-y-4 item-start xs:items-center justify-center relative group'>
                            {
                                BtnList.slice(0, BtnList.length/2).map((btn, index) => {
                                    ...
                                    return <NavButton className='absolute' key={btn.label} x={0} y={0} {...btn}/>
                                })
                            }
                            </motion.div>
                            ...
                            className='w-full px-2.5 xs:p-0 xs:w-max flex flex-col space-y-4 items-end xs:items-center justify-center relative group'>
                            {
                                BtnList.slice(BtnList.length/2, BtnList.length).map((btn, index) => {
                                    ...
                                    return <NavButton className='absolute' key={btn.label} x={0} y={0} {...btn} labelDirection="left"/>
                                })
                            }
                            </motion.div>
                        </>
